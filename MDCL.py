...
train_file='./HGA/train.csv'
train = pd.read_csv(train_file)

warm_test_file='./HGA/warm_test.csv'
warm_test = pd.read_csv(warm_test_file)

cold_test_file='./HGA/cold_test.csv'
cold_test = pd.read_csv(cold_test_file)

all_test_file='./HGA/all_test.csv'
all_test = pd.read_csv(all_test_file)

invoke_data_file =".HGA/Invoke_edge.csv"
invoke = pd.read_csv(invoke_data_file)

api_content = torch.load("./data/HGA/newf.pt")


num_api = invoke['Api_ID'].unique().shape[0]
num_mashup = invoke['Mashup_ID'].unique().shape[0]
all_nodes = num_api + num_mashup
print(num_api,num_mashup,all_nodes)


apicontent_size=api_content.size(1)


warm_api = set(warm_test['Api_ID'].unique().tolist())
cold_api = set(cold_test['Api_ID'].unique().tolist())


neg_samples_api= calculate_neg_samples_api(train,invoke,num_neg=256)
neg_samples_mashup= calculate_neg_samples_mashup(train,invoke,num_neg=256)

adj_matrix = np.zeros((all_nodes, all_nodes), dtype=np.float32)

for _, row in train.iterrows():
    mashup_idx = row["Mashup_ID"]
    api_idx = row["Api_ID"] + num_mashup  
    adj_matrix[mashup_idx, api_idx] = 1 
    adj_matrix[api_idx, mashup_idx] = 1  
...
class Recommender(nn.Module):
    ...
    def recom_level_loss(self, data, beta, neg_samples_api, neg_samples_mashup, temperature=0.1):
        ...
        mashup_embed = self.mashup_emb(mashup_indices) 
        api_embed = self.api_emb(api_indices)          
    ...
    def group_level_loss(self, data, neg_samples_api, temperature=0.1):
        ...
        for mashup_id in mashup_indices:
            ...
            softmax_weights = F.softmax(cosine_sim / temperature, dim=0) 
    ...
    def bpr_loss(self, data, neg_samples_api, temperature=0.1):
        ...
        for api in api_indices:
            neg_sample = random.choice(neg_samples_api[api.item()])  # 
        ...
        neg_sim_ui = torch.sum(mashup_embed * negative_api_embed, dim=1) / temperature  
    
        loss_ui = -torch.log(torch.sigmoid(pos_sim_ui - neg_sim_ui))  
